...
// ══════════════════════════════════════════════════
// TEAM NAME SEARCH ENDPOINT
// Direct name search across all teams in api-football.com
// Used as fallback when competition roster misses qualifying round clubs
// ══════════════════════════════════════════════════
app.get('/api/search-team', async (req, res) => {
  const { name } = req.query;
  if (!name) return res.status(400).json({ error: 'name required', teams: [] });
  try {
    const r = await afGet(`${AF_HOST}/teams`, { search: name });
    const teams = (r.data.response || []).map(t => ({
      id: t.team.id,
      name: t.team.name,
      shortName: t.team.name,
    ...
    return res.json({ teams, _source: 'af' });
  } catch (e) {
